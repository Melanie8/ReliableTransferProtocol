...
and we set the flag \lstinline|panic| to \lstinline|true|. In \lstinline|agent.c|, before each message, if the \lstinline|panic| flag is set, the rest of the messages are ignored and the thread terminates. In \lstinline|sender.c|, we join each threads, close the file and exits with status \lstinline|EXIT_FAILURE|. We don't use any \texttt{exit()}, everything goes through the return of the main.
...
\subsection{Clean termination of the agents}
Comme limitation, tu peux dire que quand il y a une erreur, tout le monde se termine normalement sauf parfois le acker
Comme il est busy receiving
Pour fix on pourrait faire un timeout pour le receiving
...
We tested the compatibility between our programs with the ones of two other groups. It allowed us to fix some bugs in our code.

\subsection{Derval-Gego}
\subsubsection{Our sender} 

grande nombre d'envoi du même packet
